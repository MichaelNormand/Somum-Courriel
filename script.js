...
let placeCaret = (content, position = undefined) => {
	if (content === undefined || content === null) {
		return
	}
	let caretPosition = undefined
	if (position !== undefined && !isNaN(position)) {
		caretPosition = (position >= 0 && position <= text.textContent.length ? position : (position < 0 ? 0 : text.textContent.length))
	}
	content.focus()
		...
		if (content.lastChild !== undefined && content.lastChild !== null) {
			...
			if (text.tagName === undefined) {
				if (caretPosition !== undefined) {
					range.setStart(text, caretPosition)
				} else {
	...
	selection = {
		...
		HasSelection: false,
		StartOffset: caretPosition !== undefined ? caretPosition : content.textContent.length,
		EndOffset: caretPosition !== undefined ? caretPosition : content.textContent.length,
	}
}
...
let policeChangeCallback = (police) => {
	...
	placeCaret(elementsToChange.pop(), selection.EndOffset)
	addContentState()
}
...
let policeTextAlignementLeftCallback = () => {
	...
	placeCaret(container.lastChild, selection.EndOffset)
	addContentState()
}
...
let initialize = (initializationContent = undefined) => {
	...
	content.addEventListener('keydown', (event) => {
		...
		if (newValue.length === 1 && !isParentImageContainer && !event.ctrlKey) {
			if (activeElement.tagName === 'P' && activeElement.children.length > 0 && activeElement.lastChild.tagName === 'SOMUM-CUSTOM-STYLE') {
				...
				placeCaret(activeElement.lastChild)
				addContentState()
				...
				if (activeSelection.type === 'Caret') {
					if (!isParentImageContainer && !isParentNumberedList && !isParentOrderedList) {
						if (activeElement.parentElement.children.length === 1 && activeElement.textContent.length === 1) {
							...
							placeCaret(containerSibling.lastChild)
						}
						...
						if (container.lastChild.tagName === 'SOMUM-CUSTOM-STYLE' && container.lastChild.textContent.length <= 0 && container.children.length === 2) {
							...
							if (container.previousSibling !== undefined && container.previousSibling !== null) {
								placeCaret(container.previousSibling)
							} else if (container.nextSibling !== undefined && container.nextSibling !== null) {
								placeCaret(container.nextSibling)
							}
							if (mainStructure.children.length >= 1 && container.parentElement.children.length > 1) {
								...
								placeCaret(container.firstChild)
							}
							...
							placeCaret(container.lastChild)
						}
					}
				} else {
					if (!isParentImageContainer) {
						...
						if (container.children.length <= 0 || ((isParentNumberedList || isParentOrderedList) && container.children.length === 1)) {
							...
							placeCaret(selection.ActiveElements[0])
						}
					} else if (isParentImageContainer) {
						let image = container.getElementsByTagName('img')[0]
						if (image === undefined || image === null) {
			...
			if (!isParentNumberedList && !isParentOrderedList) {
				...
				placeCaret(newContainer.lastChild)
			} else if (isParentNumberedList || isParentOrderedList) {
				if (container.lastChild.tagName === 'SOMUM-CUSTOM-STYLE' && container.lastChild.textContent.length <= 0 && container.children.length === 2) {
					if (isParentNumberedList) {
						let actualCount = parseInt(container.getAttribute('counter'))
	...
	document.onclick = (event) => {
		...
		if (!childSelected && content.contains(event.target)) {
			...
			if (layoutOptionToggled) {
				placeCaret(content.lastChild.lastChild.lastChild.lastChild)
				selection = {
				...
				placeCaret(content.lastChild.lastChild.lastChild)
				selection = {
	...
	if (initializationContent === undefined || initializationContent === null) {
		placeCaret(content.lastChild.lastChild.lastChild)
	} else {
		let parsedContent = new DOMParser().parseFromString(initializationContent, 'text/html')
		if (parsedContent !== undefined && parsedContent !== null && parsedContent.body.innerHTML === initializationContent) {
			let wrapper = parsedContent.body
			let contentToWork = wrapper.firstChild
			if (wrapper.firstChild.hasAttribute('layoutMode')) {
				let layoutModeCheck = wrapper.firstChild.getAttribute('layoutMode')
				let layoutModeValue = false
				let pTags = contentToWork.querySelectorAll('p')
				if (layoutModeCheck !== undefined && layoutModeCheck !== null) {
					layoutModeValue = layoutModeCheck === 'true' ? true : false
				}
				...
				pTags.forEach((element) => {
					...
					element.addEventListener('focusin', () => {
						element.style.borderBottom = '1px solid rgb(39, 92, 140)'
					})
...
if (!window.instanciateEditor) {
	window.instanciateEditor = (initializationContent) => {
		document.querySelector('#' + mainRoot).innerHTML = ''
