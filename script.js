...
document.addEventListener("DOMContentLoaded", () => {
	...
	let createPoliceElementForContent = (
		name,
		value,
		policeContent,
		policeDropdownArrow,
		...
		element.addEventListener("click", () => {
			...
			policeDropdownArrow.style.borderColor =
				"#275C8C transparent transparent transparent"
	...
	let createSizeElementForContent = (
		name,
		value,
		sizeContent,
		sizeDropdownArrow,
		...
		element.addEventListener("click", () => {
			...
			sizeDropdownArrow.style.borderColor =
				"#275C8C transparent transparent transparent"
	...
	let createIndentElementForContent = (
		name,
		image,
		indentContent,
		indentThumbnailArrow,
		callback
	) => {
		let element = document.createElement("div")
		...
		element.addEventListener("click", () => {
			...
			indentThumbnailArrow.style.borderColor =
				"#275C8C transparent transparent transparent"
	...
	let createTextAlignElementForContent = (
		name,
		image,
		textAlignementContent,
		textAlignementThumbnailArrow,
		callback
	) => {
		let element = document.createElement("div")
		element.appendChild(image)
		...
		element.addEventListener("click", () => {
			...
			textAlignementThumbnailArrow.style.borderColor =
	...
	let createRightClickMenu = (
		xPosition,
		yPosition,
		layoutOptionToggledCallback,
		colorChangeOptionToggledCallback,
		closeOptionCallback
	) => {
		...
		layoutOptionText.textContent = layoutOptionToggled
			? "Utiliser le mode de texte"
			: "Utiliser le mode cadré"
		...
		if (layoutSelected !== undefined && layoutOptionToggled) {
			...
			colorChangeOptionContainer.addEventListener("mouseenter", () => {
				colorChangeOptionContainer.style.backgroundColor =
					"rgba(0, 0, 0, 0.1)"
			})
			...
			colorChangeOptionText.textContent =
			...
			colors.forEach((element) => {
				...
				targetColor +=
					hexElement.length == 1 ? "0" + hexElement : hexElement
			})
	...
	let createPoliceSelection = (callbackPoliceChangeFunction) => {
		...
		policeContent.appendChild(
			createPoliceElementForContent(
				"Sans Serif",
				"Arial, Helvetica, sans-serif",
				policeContent,
				policeDropdownArrow,
				selectedPoliceContent,
				callbackPoliceChangeFunction
			)
		)
		policeContent.appendChild(
			createPoliceElementForContent(
		...
		let openPoliceDropdown = () => {
			...
			policeDropdownArrow.style.borderColor =
				"transparent transparent #275C8C transparent"
	...
	let createSizeSelection = (callBackSizeChange) => {
		...
		// Setting up content
		sizeContent.appendChild(
			createSizeElementForContent(
				"Small",
				"small",
				sizeContent,
				sizeDropdownArrow,
	...
	let createIndentSelection = (
		callbackAddIndentChange,
		callbackRemoveIndentChange
	) => {
		...
		indentThumbnailContainer.appendChild(
			indentRightThumbnail.cloneNode(true)
		)
		indentThumbnailContainer.appendChild(indentThumbnailArrow)

		// Adding the options the the content of the dropdown
		indentContent.appendChild(
			createIndentElementForContent(
		...
		indentContent.appendChild(
			createIndentElementForContent(
				...
				indentLeftThumbnail.cloneNode(true),
				indentContent,
				indentThumbnailArrow,
				callbackRemoveIndentChange
			)
	...
	let createTextAlignementSelection = (
		callbackTextAlignLeftChange,
		callbackTextAlignCenterChange,
		callbackTextAlignRightChange
	) => {
		...
		// Adding the content to the thumbnail container
		textAlignThumbnailContainer.appendChild(
			textAlignLeftThumbnail.cloneNode(true)
		)
		textAlignThumbnailContainer.appendChild(textAlignThumbnailArrow)

		// Adding the options the the content of the dropdown
		textAlignContent.appendChild(
			createTextAlignElementForContent(
				"text-align-left-container",
				textAlignLeftThumbnail.cloneNode(true),
				textAlignContent,
				textAlignThumbnailArrow,
				callbackTextAlignLeftChange
			)
		)
		textAlignContent.appendChild(
			createTextAlignElementForContent(
				...
				textAlignCenterThumbnail.cloneNode(true),
				textAlignContent,
				textAlignThumbnailArrow,
				callbackTextAlignCenterChange
			)
		...
		textAlignContent.appendChild(
			createTextAlignElementForContent(
				...
				textAlignRightThumbnail.cloneNode(true),
				textAlignContent,
				textAlignThumbnailArrow,
				callbackTextAlignRightChange
			)
	...
	let goBackInContentState = () => {
		...
		if (contentIndex > 0) {
			...
			for (
				let i = 0;
				i < contentState[contentIndex - 1].content.children.length;
				i++
			) {
				content.appendChild(
					contentState[contentIndex - 1].content.children[
						i
					].cloneNode(true)
				)
			}
			layoutOptionToggled =
				contentState[contentIndex - 1].layoutOptionToggled
	...
	let truncateSelection = () => {
		if (
			selection === undefined ||
			selection.ActiveElements === undefined ||
			selection.ActiveElements.length <= 0
		) {
			return
		}
		if (!selection.HasSelection) {
			...
			if (loneElement.tagName === "P") {
				...
				if (
					container.lastChild === loneElement &&
					selection.StartOffset === loneElement.textContent.length
				) {
		...
		if (selection.ActiveElements.length === 1) {
			...
			if (selection.StartOffset > 0) {
				firstTextElement = element.textContent.substring(
					0,
					selection.StartOffset
				)
			}
			if (selection.EndOffset < element.textContent.length) {
				lastTextElement = element.textContent.substring(
					selection.EndOffset,
					element.textContent.length
				)
			}
			selectedTextElement = element.textContent.substring(
				selection.StartOffset,
				selection.EndOffset
			)
			if (
				selectedTextElement !== undefined &&
				selectedTextElement.length > 0
			) {
				let selectedElement = document.createElement(
					"somum-custom-style"
				)
				if (element.tagName === "P") {
					...
					selectedElement.innerHTML = selectedTextElement
						.split(" ")
						.join("&nbsp;")
					if (
						firstTextElement !== undefined &&
						firstTextElement.length > 0
					) {
						let firstElement = selectedElement.cloneNode(true)
						firstElement.textContent = firstTextElement
						firstElement.innerHTML = firstTextElement
							.split(" ")
							.join("&nbsp;")
					...
					if (
						...
						lastTextElement.length > 0
					) {
						let lastElement = selectedElement.cloneNode(true)
						lastElement.textContent = lastTextElement
						lastElement.innerHTML = lastTextElement
							.split(" ")
							.join("&nbsp;")
						if (
							selectedElement.nextSibling !== undefined &&
							...
							selectedElement.parentElement.appendChild(
								lastElement
							)
						}
					}
			...
			let lastElement =
				selection.ActiveElements[selection.ActiveElements.length - 1]
			let firstSelectedText = firstElement.textContent.substring(
				selection.StartOffset,
				firstElement.textContent.length
			)
			let lastSelectedText = lastElement.textContent.substring(
				0,
				selection.EndOffset
			)
			if (
				firstSelectedText === undefined ||
				firstSelectedText.length <= 0 ||
				lastSelectedText === undefined ||
				lastSelectedText.length <= 0
			) {
				return undefined
			}
			if (
				selection.StartOffset > 0 &&
				firstElement.textContent.length > 0
			) {
				let notSelectedText = firstElement.textContent.substring(
					0,
					selection.StartOffset
				)
				let notSelectedElement = firstElement.cloneNode(true)
				notSelectedElement.textContent = notSelectedText
				notSelectedElement.innerHTML = notSelectedText
					.split(" ")
					.join("&nbsp;")
			...
			if (
				selection.EndOffset < lastElement.textContent.length &&
				lastElement.textContent.length > 0
			) {
				let notSelectedText = lastElement.textContent.substring(
					selection.EndOffset,
				...
				notSelectedElement.innerHTML = notSelectedText
					...
					.join("&nbsp;")
				if (
					lastElement.nextSibling !== undefined &&
	...
	let policeTextAlignementLeftCallback = () => {
		...
		let isParentNumberedList =
			container.parentElement.getAttribute("isnumberedlist") === null
				? false
				: true
		let isParentOrderedList =
			container.parentElement.getAttribute("isorderedlist") === null
				? false
				: true
	...
	let policeAddNumberedListCallback = () => {
		...
		if (
			...
			container.children[0].tagName === "BR"
		) {
			container.removeChild(container.children[0])
		} else if (
			container.children.length > 1 ||
			(container.children.length === 1 &&
				container.children[0].textContent.length > 0)
		) {
			addContainer = true
		}
		let mainContainer = addContainer
			? document.createElement("div")
			: container.parentElement
		let numberedList = addContainer
			? document.createElement("p")
			: container
		let firstElement = addContainer
			? document.createElement("somum-custom-style")
		...
		let mainContentWidth =
			parseInt(mainContent.style.width.replace("px", "")) - 21
		...
		if (addContainer) {
			if (
				container.parentElement.nextSibling !== undefined &&
				container.parentElement.nextSibling !== null
			) {
	...
	let addImageCallback = (image) => {
		...
		let isOnlySpace =
			container.children[0].textContent.replace(/\s/g, "").length < 1
		if (
			container.children[0] !== undefined &&
			container.children.length === 1 &&
			container.children[0].tagName === "BR"
		...
		let mainContentWidth =
			parseInt(mainContent.style.width.replace("px", "")) - 21
		let imageContainer = addContainer
			? document.createElement("div")
			: container.parentElement
		let mainContainer = addContainer
			? document.createElement("p")
	...
	let initializeContentInTextMode = (container, mainContainer) => {
		...
		let containerWidth =
			parseInt(mainContainer.style.width.replace("px", "")) - 21
	...
	let initialize = (initializationContent = undefined) => {
		...
		document.addEventListener("selectionchange", () => {
			...
			if (
				toolbar.contains(activeSelection.anchorNode) ||
				activeSelection.anchorNode === undefined ||
				activeSelection.anchorNode === null ||
				selectionAlreadyChanged
			) {
			...
			if (activeSelection.type === "Caret") {
				...
				if (
					activeSelection.anchorNode.tagName === undefined &&
				...
				if (
					activeTag !== undefined &&
					activeTag !== null &&
					activeTag.tagName === "SOMUM-CUSTOM-STYLE"
				) {
					selection = {
			...
			if (
				...
				activeSelection.anchorNode === activeSelection.focusNode &&
				firstNode.tagName === "SOMUM-CUSTOM-STYLE"
			) {
				selection = {
					ActiveElements: [firstNode],
					HasSelection: true,
					StartOffset:
						activeSelection.anchorOffset <
						activeSelection.focusOffset
							? activeSelection.anchorOffset
							: activeSelection.focusOffset,
					EndOffset:
						activeSelection.anchorOffset <
			...
			let endPoint =
				firstNodeIndex < secondNodeIndex
					...
					: firstNodeIndex
			let lastElement =
				firstNodeIndex < secondNodeIndex ? secondNode : firstNode
			...
			for (let i = startPoint; i <= endPoint; i++) {
				...
				if (
					container.children[i].tagName !== "SOMUM-CUSTOM-STYLE" &&
					i === startPoint
				) {
					startOffset = 0
				}
		...
		content.addEventListener("keydown", (event) => {
			...
			let isParentOrderedList =
				container.parentElement.getAttribute("isorderedlist") === null
					? false
					: true
			let isParentImageContainer =
				container.parentElement.getAttribute("isimagecontainer") ===
				null
					? false
					: true
			if (
				...
				if (
					...
					activeElement.lastChild.tagName === "SOMUM-CUSTOM-STYLE"
				) {
					event.preventDefault()
					activeElement.lastChild.textContent =
						activeElement.lastChild.textContent +
						newValue.split(" ").join("&nbsp;")
					...
					activeElement.tagName === "P" &&
					(activeElement.children.length <= 0 ||
						(activeElement.children[0] !== undefined &&
							activeElement.children[0].tagName === "BR"))
				) {
					event.preventDefault()
					if (activeElement.children[0] !== undefined) {
						activeElement.removeChild(activeElement.children[0])
					}
					let firstElement = document.createElement(
						"somum-custom-style"
					)
					...
					activeElement.tagName === "SOMUM-CUSTOM-STYLE" &&
					activeElement.textContent.length <= 0 &&
					activeElement.innerHTML.length <= 0
				) {
					event.preventDefault()
				...
				if (activeElement.tagName === "SOMUM-CUSTOM-STYLE") {
					if (activeSelection.type === "Caret") {
						if (
							!isParentImageContainer &&
							!isParentNumberedList &&
							!isParentOrderedList
						) {
							if (
								activeElement.parentElement.children.length ===
									1 &&
								activeElement.textContent.length === 1
							) {
								event.preventDefault()
							...
							event.preventDefault()
							imageContainer.removeChild(image)
							container.parentElement.removeAttribute(
								"isimagecontainer"
							)
							container.parentElement.style.minHeight = "20px"
							placeCaretAtEnd(imageContainer)
							...
							if (
								...
								if (isParentNumberedList) {
									let actualCount = parseInt(
										container.getAttribute("counter")
									)
									let elementCountBuffer
									for (
										let i = actualCount - 1;
										i <
										container.parentElement.children.length;
										i++
									) {
										elementCountBuffer = parseInt(
										...
										container.parentElement.children[
											i
										].firstChild.textContent = `${elementCountBuffer}. `
										container.parentElement.children[
											i
										].firstChild.innerHTML = `${elementCountBuffer}.&nbsp;`
										container.parentElement.children[
											i
										].setAttribute(
											"counter",
								...
								if (
									container.previousSibling !== undefined &&
									container.previousSibling !== null
								) {
									placeCaretAtEnd(container.previousSibling)
								} else if (
									container.nextSibling !== undefined &&
									container.nextSibling !== null
								) {
									placeCaretAtEnd(container.nextSibling)
								}
								if (
									mainStructure.children.length >= 1 &&
									container.parentElement.children.length > 1
								) {
									if (
										container.parentElement.lastChild ===
										container
									) {
										let mainContainer =
											container.parentElement
										let mainContainerSibling =
											mainContainer.nextSibling
										let newMainContainer = document.createElement(
											"div"
										)
										let newContainer = document.createElement(
											"p"
										)
										...
										let containerWidth =
											parseInt(
												content.style.width.replace(
													"px",
													""
												)
											) - 21
										newMainContainer.style.display = "flex"
										newMainContainer.style.flexDirection =
											"column"
										newMainContainer.style.minHeight =
											"20px"
										...
										newContainer.addEventListener(
											"focusin",
											() => {
												newContainer.style.borderBottom =
													"1px solid rgb(39, 92, 140)"
											}
										)
										newContainer.addEventListener(
											"focusout",
										...
										newMainContainer.appendChild(
											newContainer
										)
										if (
											mainContainerSibling !==
												undefined &&
											mainContainerSibling !== null
										) {
											mainStructure.insertBefore(
												newMainContainer,
												mainContainerSibling
											)
										} else {
											mainStructure.appendChild(
												newMainContainer
											)
										}
								...
								container.lastChild.tagName ===
									"SOMUM-CUSTOM-STYLE" &&
								container.lastChild.textContent.length === 1 &&
								container.children.length === 2
							) {
								event.preventDefault()
						...
						if (!isParentImageContainer) {
							...
							if (selection.ActiveElements.length > 1) {
								let remainingText = selection.ActiveElements[0].textContent.substring(
									0,
									selection.StartOffset
								)
								if (remainingText.length <= 0) {
									selection.ActiveElements[0].parentElement.removeChild(
										selection.ActiveElements[0]
									)
								}
								selection.ActiveElements[0].textContent = remainingText
								selection.ActiveElements[0].innerHTML = remainingText
									.split(" ")
									.join("&nbsp;")
								remainingText = selection.ActiveElements[
									selection.ActiveElements.length - 1
								...
								for (
									let i = 1;
									i < selection.ActiveElements.length - 1;
									i++
								) {
									selection.ActiveElements[
										i
									].parentElement.removeChild(
										selection.ActiveElements[i]
									)
								}
							} else {
								let firstPart = selection.ActiveElements[0].textContent.substring(
									0,
									selection.StartOffset
								)
								let lastPart = selection.ActiveElements[0].textContent.substring(
									selection.EndOffset,
									selection.ActiveElements[0].textContent
										.length
								)
				...
				if (!isParentNumberedList && !isParentOrderedList) {
					...
					let containerWidth =
						parseInt(content.style.width.replace("px", "")) - 21
					let elementsToAdd = []
					let activeSelection = window.getSelection()
					...
					if (
						...
						(selection.StartOffset !==
							selection.ActiveElements[0].textContent.length ||
							selection.ActiveElements[0].parentElement
								.lastChild !== selection.ActiveElements[0]) &&
						!isParentImageContainer
					) {
						...
						newElement.innerHTML = textToAdd
							.split(" ")
							.join("&nbps;")
						selection.ActiveElements[0].textContent = selection.ActiveElements[0].textContent.substring(
							0,
							selection.StartOffset
						)
						selection.ActiveElements[0].innerHTML = selection.ActiveElements[0].textContent
							.substring(0, selection.StartOffset)
							.split(" ")
							.join("&nbsp;")
					...
					if (
						mainContainerSibling !== undefined &&
						mainContainerSibling !== null
					) {
						mainStructure.insertBefore(
							newMainContainer,
					...
					if (
						container.lastChild.tagName === "SOMUM-CUSTOM-STYLE" &&
						container.lastChild.textContent.length <= 0 &&
						container.children.length === 2
					) {
						if (isParentNumberedList) {
							...
							if (actualCount > 1) {
								for (
									let i = actualCount - 1;
									i < container.parentElement.children.length;
									i++
								) {
									elementCountBuffer = parseInt(
										container.parentElement.children[
						...
						if (
							...
							container.parentElement.previousSibling !==
								undefined &&
							container.parentElement.previousSibling !== null
						) {
							placeCaretAtEnd(
								container.parentElement.previousSibling
							)
							...
							container.parentElement.nextElementSibling !==
								undefined &&
							container.parentElement.nextElementSibling !== null
						) {
							placeCaretAtEnd(
								container.parentElement.nextElementSibling
							)
						...
						if (
							...
							if (
								container.parentElement.lastChild === container
							) {
								let mainContainer = container.parentElement
								let mainStructure = mainContainer.parentElement
								let mainContainerSibling =
								...
								let containerWidth =
									parseInt(
										content.style.width.replace("px", "")
									) - 21
								newMainContainer.style.display = "flex"
								newMainContainer.style.flexDirection = "column"
								newMainContainer.style.minHeight = "20px"
						...
						if (isParentNumberedList) {
							...
							for (
								...
								container.parentElement.children[
									i
								].setAttribute("counter", elementCountBuffer)
							}
						} else {
		...
		underlineSelection = createUnderlineSelection(
			policeUnderlineToggleCallback
		)
		textAlignmentSelection = createTextAlignementSelection(
			policeTextAlignementLeftCallback,
			policeTextAlignementCenterCallback,
			policeTextAlignementRightCallback
		)
		indentSelection = createIndentSelection(
			policeAddIndentCallback,
			policeRemoveIndentCallback
		)
		numberedListSelection = createNumberedListSelection(
			policeAddNumberedListCallback
		)
		orderedListSelection = createOrderedListSelection(
			policeAddOrderedListCallback
		)
		...
		if (window.screen.width >= 680) {
			...
			morePoliceOptionsDropdown.style.border =
				"1px solid rgba(0, 0, 0, 0.1)"
			morePoliceOptionsDropdown.style.boxShadow =
				"lightgray 20px 20px 50px"
		...
		document.onclick = (event) => {
			...
			if (
				window.closePoliceDropdown &&
				!policeSelection.contains(event.target) &&
				policeDropdownToggled
			) {
				window.closePoliceDropdown()
			}
			if (
				window.closeSizeDropdown &&
				!sizeSelection.contains(event.target) &&
				sizeDropdownToggled
			) {
				window.closeSizeDropdown()
			}
			if (
				window.closeTextAlignDropdown &&
				!textAlignmentSelection.contains(event.target) &&
				textAlignDropdownToggled
			) {
				window.closeTextAlignDropdown()
			}
			if (
				window.closeIndentDropdown &&
				!indentSelection.contains(event.target) &&
				indentDropdownToggled
			) {
				window.closeIndentDropdown()
			}
			if (
				morePoliceOptionsSelection &&
				window.closeMorePoliceOptionsDropdown &&
				!morePoliceOptionsSelection.contains(event.target) &&
				morePoliceDropdownToggled
			) {
				window.closeMorePoliceOptionsDropdown()
			}
			if (
				layoutOption !== undefined &&
				!layoutOption.contains(event.target)
			) {
				document
					.getElementsByTagName("body")[0]
					.removeChild(layoutOption)
			...
			if (!childSelected && content.contains(event.target)) {
				...
				if (layoutOptionToggled) {
					placeCaretAtEnd(
						content.lastChild.lastChild.lastChild.lastChild
					)
					selection = {
						ActiveElements: [
							content.lastChild.lastChild.lastChild.lastChild,
					...
					selection = {
						...
						HasSelection: false,
						StartOffset:
							content.lastChild.lastChild.lastChild.textContent
								.length,
						EndOffset:
							content.lastChild.lastChild.lastChild.textContent
								.length,
		...
		if (!window.onresize) {
			window.onresize = () => {
				if (
					(window.screen.width < 680 && latestScreenWidth >= 680) ||
					(window.screen.width >= 680 && latestScreenWidth < 680)
				) {
					root.innerHTML = ""
					initialize()
		...
		if (!window.instanciateEditor) {
			window.instanciateEditor = (initializationContent) => {
				root.innerHTML = ""
				initialize(initializationContent)
		...
		if (
			...
			if (
				...
				parsedContent !== null &&
				parsedContent.body.innerHTML === initializationContent
			) {
				let wrapper = parsedContent.body
				let contentToWork = wrapper.firstChild
				if (wrapper.firstChild.hasAttribute("layoutMode")) {
					let layoutModeCheck = wrapper.firstChild.getAttribute(
						"layoutMode"
					)
					let layoutModeValue = false
					let pTags = contentToWork.querySelectorAll("p")
					if (
						layoutModeCheck !== undefined &&
						layoutModeCheck !== null
					) {
						layoutModeValue =
							layoutModeCheck === "true" ? true : false
					}
					layoutOptionToggled = layoutModeValue
                    ...
                    if (layoutModeValue) {
                        let header = wrapper.firstChild.firstChild.firstChild
                    ...
                    content.innerHTML = ''
                    for (let i = 0; i < contentToWork.children.length; i++) {
                        content.appendChild(contentToWork.children[i])
                    }
				}
			} else {
				placeCaretAtEnd(content.lastChild.lastChild.lastChild)
